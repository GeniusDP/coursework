...
const RegisterForm = () => {
  ...
  const userExists = async (username) => {
    const url = process.env.REACT_APP_USER_MANAGEMENT_URL
        + `/exists-by-username?username=${username}`;
    try {
      const response = await fetch(url);
      setUserAlreadyExists(response.ok);
      return response.ok;
    } catch (error) {
  ...
  }, [username, password, email, firstName, lastName, userAlreadyExists]);
  ...
  async function performRegister() {
    ...
    try {
      const response = await customFetch(
          process.env.REACT_APP_USER_MANAGEMENT_URL + "/register",
          body
      );
      if (response.ok) {
        setRedirect(true);
        ...
        <div className="register-form-wrapper">
          ...
          <div className="register-form">
            ...
            <div>
              ...
              {userAlreadyExists && (
                  <div style={{color: "red"}}>Such user already exists</div>
              )}
              {username.isDirty &&
